...
function ToastItem({ toast, index, onRemove, isTop }: ToastItemProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "pointer-events-auto relative min-w-[320px] max-w-[420px] rounded-lg border border-slate-800 border-l-4 bg-slate-950/95 p-4 shadow-xl text-slate-100 backdrop-blur-md",
        borderColors[type],
      )}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          {title && <p className="font-semibold text-slate-50">{title}</p>}
          <p className={cn("text-slate-300 text-sm", title && "mt-1")}>
            {message}
          </p>
          {action && (
            <button
              onClick={action.onClick}
              className="mt-2 font-medium text-amber-400 text-sm hover:underline"
            >
              {action.label}
            </button>
          )}
        ...
        <button
          onClick={onRemove}
          className="flex-shrink-0 rounded-md p-1 text-slate-400 transition-colors hover:bg-slate-800 hover:text-slate-100"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className={cn(
            "fixed bottom-8 left-1/2 z-[9999] rounded-full px-6 py-3 font-semibold text-slate-50 text-sm shadow-xl border border-slate-700/50 backdrop-blur-md",
            bgColors[type],
          )}
        >
          {message}
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed bottom-8 left-1/2 z-[9999] overflow-hidden rounded-lg bg-slate-900 border border-slate-800 text-slate-100 shadow-xl"
        >
          <div className="flex items-center gap-4 px-4 py-3">
            ...
            <button
              ...
              className="rounded-md bg-amber-500 px-3 py-1 font-bold text-slate-950 text-xs transition-opacity hover:opacity-90"
            >
              Hoàn tác
            </button>
          </div>
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed top-4 right-4 z-[9999] w-80 overflow-hidden rounded-xl border border-slate-800 bg-slate-950/95 text-slate-100 shadow-2xl backdrop-blur-md"
        >
          <div className="p-4">
            <div className="flex items-start gap-3">
              {avatar ? (
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 line-clamp-2 text-slate-300 text-sm">
                  {message}
                </p>
              </div>
            </div>
          ...
          <button
            onClick={onClose}
            className="absolute top-2 right-2 rounded-full p-1 transition-colors hover:bg-slate-800"
          >
      ...
      <AnimatePresence mode="popLayout">
        {visibleToasts.map((toast, index) => (
          <motion.div
            ...
            className="rounded-lg border border-slate-800 bg-slate-900 p-4 shadow-lg text-slate-100"
          >
            <div className="flex items-start gap-3">
              {icons[toast.type || "default"]}
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-slate-50">{toast.title}</p>
                <p className="mt-0.5 text-slate-300 text-sm">{toast.message}</p>
              </div>
              <button
                onClick={() => onRemove(toast.id)}
                className="rounded-md p-1 transition-colors hover:bg-slate-850"
              >
                <X className="h-4 w-4 text-slate-400" />
              </button>
            </div>
