      ...
      >
        ...
        <div className="flex flex-col items-center gap-1">
          <button
            ...
            disabled={disabled}
            className="pointer-events-auto rounded-lg p-1.5 transition-colors hover:bg-slate-800 disabled:opacity-50"
          ...
          <input
            ...
            disabled={disabled}
            className={cn(
              "pointer-events-auto w-12 rounded border-none bg-transparent text-center font-bold font-mono focus:outline-none focus:ring-2 focus:ring-amber-500",
              sizes.header,
            )}
            aria-label="Hours"
            maxLength={2}
        ...
        <span className={cn("font-bold text-slate-500", sizes.header)}>:</span>
        {/* Minutes */}
        <div className="flex flex-col items-center gap-1">
          <button
            type="button"
        ...
        {!use24Hour && (
          <button
            ...
            disabled={disabled}
            className="pointer-events-auto ml-2 rounded-lg bg-slate-800 px-3 py-2 font-semibold text-sm text-slate-100 transition-colors hover:bg-slate-700 disabled:opacity-50"
        ...
        {localeStrings.monthsShort.map((month, index) => {
          ...
          return (
            <motion.button
              ...
              disabled={isDisabled}
              className={cn(
                "rounded-lg px-2 py-3 font-medium text-sm transition-all focus:outline-none focus:ring-2 focus:ring-amber-500",
                isSelected
      ...
      <div className="space-y-2 p-2" role="listbox" aria-label="Select year">
        <div className="mb-2 flex items-center justify-between">
          <button
            ...
            className="rounded-lg p-1.5 transition-colors hover:bg-slate-800 text-slate-300"
...
const PresetsPanel = React.memo(
  ({
    ...
    <div className="mr-3 min-w-[160px] max-w-[220px] space-y-1 border-slate-800 border-r pr-3 self-start max-h-[70vh] overflow-auto" role="group" aria-label="Quick date presets">
      <span className="mb-2 block font-bold text-slate-500 text-[10px] uppercase tracking-wider">
        Chọn nhanh
      </span>
      {presets.map((preset, index) => (
        <motion.button
          ...
          disabled={disabled}
          className="w-full rounded-lg px-2.5 py-1.5 text-left text-xs text-slate-300 transition-colors hover:bg-slate-800 hover:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500 disabled:opacity-50"
        >
          {preset.label}
        </motion.button>
      ))}
...
const CalendarContent = React.memo(
  ({
    ...
    onClose,
    id,
  }: InternalCalendarProps & { onClose?: () => void; localeStrings: CalendarLocale }) => {
    ...
    const handleViewChange = useCallback(
      (newView: CalendarView) => {
        ...
        announce(`Chuyển sang chế độ xem ${newView}`);
      },
        ...
        if (mode === "single") {
          const dateToSet =
            showTime && value instanceof Date
              ...
              : day;
          onChange?.(dateToSet);
          announce(`Đã chọn ${format(dateToSet, "PPPP", { locale })}`);
          if (closeOnSelect && !showTime) onClose?.();
        } else if (mode === "range") {
          if (!rangeStart) {
            setRangeStart(day);
            onChange?.({ from: day, to: undefined });
            announce(`Bắt đầu khoảng: ${format(day, "PP", { locale })}`);
          } else {
            ...
            announce(
              `Khoảng chọn từ: ${format(range.from, "PP", { locale })} đến ${format(
                range.to,
                "PP",
                { locale },
              )}`,
          ...
          onChange?.(newDates);
          announce(
            `${exists ? "Bỏ chọn" : "Đã chọn"} ${format(day, "PP", {
              locale,
    ...
    useEffect(() => {
      const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if (handled) {
          ...
          announce(format(newDate, "EEEE, d MMMM, yyyy", { locale }));
        }
      };
    ...
    const handlePresetSelect = useCallback(
      (range: DateRange) => {
        ...
        announce(
          `Đã chọn khoảng preset: ${
            range.from && range.to
              ? `${format(range.from, "PP")} đến ${format(range.to, "PP")}`
              : ""
          }`,
        );
        if (closeOnSelect) onClose?.();
    ...
    const handleTimeChange = useCallback(
      (newDate: Date) => {
        if (mode === "single") {
          ...
          announce(`Thời gian được chỉnh: ${format(updatedDate, use24Hour ? "HH:mm" : "hh:mm a")}`);
        }
      },
        ...
        const dayContent = (
          <motion.button
            ...
            aria-label={`${format(day, "EEEE, d MMMM, yyyy", { locale })}${
              isSelected ? ", đã chọn" : ""
            }${isTodayDate ? ", hôm nay" : ""}${highlight ? `, ${highlight.label}` : ""}`}
            ...
            className={cn(
              ...
              !isSelected && isCurrentMonth && !inRange && "text-slate-100 hover:bg-slate-800",
              isSelected && "bg-amber-500 text-slate-950 font-bold shadow-md shadow-amber-500/25",
              isTodayDate &&
                !isSelected &&
                "ring-2 ring-amber-500/50 ring-offset-2 ring-offset-slate-900",
              inRange && "rounded-none bg-amber-500/10 text-amber-300",
              isFocused && "ring-2 ring-amber-400 ring-offset-1",
            )}
          >
          ...
          >
            {isSecondary && (
              <div className="mb-2 flex h-8 items-center justify-center">
                <span className={cn("font-bold text-slate-100", sizes.header)}>
            ...
            >
              {showWeekNumbers && (
                <div
                  className={cn(
                    sizes.cell,
                    "flex items-center justify-center font-bold text-slate-500 text-xs",
                  )}
                  role="columnheader"
                  tabIndex={-1}
                >
              ...
              {getWeekDays().map((day, i) => (
                <div
                  ...
                  className={cn(
                    sizes.cell,
                    "flex items-center justify-center font-bold text-slate-400 text-xs",
                  )}
                  tabIndex={-1}
                >
                  {day}
    ...
    return (
      <motion.div
        ...
        transition={{ duration: prefersReducedMotion ? 0 : 0.2 }}
        className={cn(
          "pointer-events-auto border border-slate-800 bg-slate-950 p-4 shadow-2xl focus:outline-none focus:ring-2 focus:ring-amber-500 rounded-xl",
          sizes.container,
          calendarWidth,
        ...
        <div className="flex-1">
          ...
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-0.5">
              <button
                ...
                disabled={disabled}
                className="rounded-lg p-1.5 transition-colors hover:bg-slate-850 text-slate-400 hover:text-slate-100"
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={disabled}
                className={cn(
                  "rounded-lg px-2 py-1 font-bold text-slate-100 transition-colors hover:bg-slate-850",
                  sizes.header,
                )}
                aria-label={`Chọn tháng, đang là ${format(currentMonth, "MMMM", { locale })}`}
              >
              ...
              <button
                ...
                aria-label={`Chọn năm, đang là ${format(currentMonth, "yyyy")}`}
              >
          ...
          {showTime && mode === "single" && view === "days" && (
            <button
              ...
              disabled={disabled}
              className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-slate-800 py-2 font-medium text-sm text-slate-200 transition-colors hover:bg-slate-700 disabled:opacity-50"
            >
          ...
          {(showTodayButton || showClearButton || (mode === "multiple" && value)) &&
            view === "days" && (
              <div className="mt-4 flex items-center justify-between border-slate-800 border-t pt-3">
                <div className="flex items-center gap-2">
                  {showTodayButton && (
                    <button
                      ...
                      onClick={goToToday}
                      disabled={disabled}
                      className="flex items-center gap-1 rounded-md px-3 py-1.5 font-semibold text-xs text-slate-300 transition-colors hover:bg-slate-800 disabled:opacity-50"
                    >
                ...
                {showClearButton && value && (
                  <button
                    ...
                    onClick={handleClear}
                    disabled={disabled}
                    className="flex items-center gap-1 rounded-md px-3 py-1.5 font-medium text-slate-400 text-xs transition-colors hover:bg-slate-800 disabled:opacity-50"
                  >
                    <X className="h-3 w-3" />
                    {localeStrings.clear}
                  </button>
...
export function AnimatedCalendar({
  ...
  const getDisplayValue = useMemo(() => {
    ...
    if (mode === "single" && value instanceof Date) {
      const fmt = formatStr || (showTime ? (use24Hour ? "dd/MM/yyyy HH:mm" : "dd/MM/yyyy hh:mm a") : "dd/MM/yyyy");
      return format(value, fmt, { locale });
    }
    if (mode === "range") {
      const range = value as DateRange;
      if (range.from && range.to) {
        return `${format(range.from, "dd/MM/yyyy", { locale })} - ${format(range.to, "dd/MM/yyyy", {
          locale,
        })}`;
      }
      if (range.from) return `${format(range.from, "dd/MM/yyyy", { locale })} - ...`;
      return placeholder;
    }
    if (mode === "multiple" && Array.isArray(value)) {
      if (value.length === 0) return placeholder;
      const firstDate = value[0];
      if (value.length === 1 && firstDate) return format(firstDate, "dd/MM/yyyy", { locale });
    ...
    <div className="relative">
      <Popover open={isOpen} onOpenChange={handleOpenChange}>
        <PopoverTrigger
          ...
          className={cn(
            sizeClasses[size],
            "flex items-center justify-start text-left font-normal border border-slate-700 bg-slate-950/70 text-slate-100 hover:bg-slate-900 hover:text-slate-100 rounded-md px-3",
        ...
        <PopoverContent className="inline-block w-auto border-0 bg-transparent p-0 shadow-none z-9999 max-h-[70vh] overflow-auto" align="start">
          <CalendarContent
      ...
      {/* Error message */}
      {error && errorMessage && (
        <p id={errorId} className="mt-1.5 flex items-center gap-1 text-red-500 text-xs">
