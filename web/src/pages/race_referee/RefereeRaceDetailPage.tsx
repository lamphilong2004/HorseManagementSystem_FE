...
import { buildRaceSimulationPlans, getStreamHorseId } from '@/utils/liveStreamSimulation'
...
export function RefereeRaceDetailPage() {
  ...
  function autoFillRankings() {
    if (horses.length === 0 || !race) return

    // Get simulation plans matching the livestream page exactly
    const plans = buildRaceSimulationPlans(race, horses)

    // Match each horse to its deterministic finish time
    const horseResults = horses.map((h, index) => {
      const streamId = getStreamHorseId(h, index)
      const plan = plans.find(p => p.streamId === streamId)
      const finishTimeSec = plan ? plan.finishTime : 23.0
    ...
    const generated = horseResults.map((item, idx) => {
      ...
      const milliseconds = Math.round((currentTime % 1) * 1000)
