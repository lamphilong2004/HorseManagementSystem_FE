...
export function PredictionsPage() {
  ...
  const filteredHistory = [...predictions]
    .filter((prediction) => {
      ...
      if (historySearchQuery.trim() !== '') {
        ...
        const matchesRace = prediction.raceId?.name?.toLowerCase().includes(query)
        const matchesHorse = prediction.horseId?.name?.toLowerCase().includes(query)
  ...
  const predictionsColumns: ColumnDef<PredictionItem & { id: string }>[] = [
    {
      id: 'race',
      header: 'Cuộc đua',
      cell: (prediction: any) => {
  ...
  const filteredHistoryWithId = filteredHistory.map((prediction) => ({
    ...prediction,
    id: prediction._id,
  }))
