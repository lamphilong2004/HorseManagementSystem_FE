...
import { BarChart2, Bell, Flag, LogOut, Medal, Radio, Trophy, Wallet } from 'lucide-react-native';
import * as api from '../../api';
import { useAuth } from '../../context/AuthContext';
import { Race, Tournament } from '../../types';
import { EmptyState, ScreenHeader, StatTile, Surface } from '../../components/MobileUI';
import { formatDateTime, formatPoints, getRaceId, getTournamentId, isActiveTournament, isLiveRace, isPredictionRaceStatus } from '../../utils/spectator';
...
export default function HomeScreen() {
  ...
  const activeTournaments = useMemo(() => tournaments.filter(isActiveTournament).slice(0, 3), [tournaments]);
  const openRaces = useMemo(() => races.filter((race) => isPredictionRaceStatus(race.status)).slice(0, 5), [races]);
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      <ScreenHeader
        title="Dashboard"
        subtitle={`Xin chào, ${user?.name || 'khán giả'}`}
        right={
      ...
      >
        ...
        <View className="flex-row flex-wrap justify-between mb-6">
          {shortcuts.map(({ label, route, Icon, color }) => (
            <TouchableOpacity
              activeOpacity={0.82}
              key={route}
