...
class ApiService {
  ...
  Future<List<Tournament>> getTournaments() async {
    ...
    return _extractList(response.data, 'tournaments')
        .map(Tournament.fromApi)
        .toList();
  }
  ...
  Future<List<Horse>> getHorses() async {
    final response = await _client.get('/api/horses');
    return _extractList(response.data, null).map(Horse.fromDirect).toList();
  }

  Future<List<Invite>> getInvites() async {
    final response = await _client.get('/api/invites');
    return _extractList(response.data, null).map(Invite.fromDirect).toList();
  }

  Future<List<Prediction>> getPredictions() async {
    final response = await _client.get('/me/predictions');
    return _extractList(response.data, 'predictions')
        .map(Prediction.fromApi)
        .toList();
  }

  Future<List<AdminUser>> getAdminUsers() async {
    final response = await _client.get('/api/admin/users');
    return _extractList(response.data, null).map(AdminUser.fromDirect).toList();
  }

  Future<List<Race>> getRefereeRaces() async {
    final response = await _client.get('/api/referee/races');
  ...
  Future<dynamic> placePrediction({
    ...
    final response = await _client.post('/races/$raceId/predictions', {
      'horseId': horseId,
      'betAmount': betAmount,
    });
  ...
  Future<List<RaceHorse>> getRaceHorses(String raceId) async {
    ...
    return _extractList(response.data, 'horses')
        .map(RaceHorse.fromRaceEntry)
        .toList();
  }

  Future<Map<String, dynamic>> getRaceResults(String raceId) async {
    final response = await _client.get('/races/$raceId/results');
    return Map<String, dynamic>.from(response.data as Map);
  }
  ...
  Future<List<Map<String, dynamic>>> getNotifications() async {
    ...
    final list = data is Map<String, dynamic> ? data['notifications'] ?? data : data;
  ...
  Future<Map<String, dynamic>> getTournamentLeaderboard(String tournamentId) async {
    final response = await _client.get('/tournaments/$tournamentId/leaderboard');
    return Map<String, dynamic>.from(response.data as Map);
  }
